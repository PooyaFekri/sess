      ...
      <div v-if="dialogFlag" class="text-center">
        <v-dialog v-model="dialogFlag" width="700">
          <v-card class="ticket_background">
            <v-card-title
              class="text-h5 lighten-2 ticket_title_background mb-5 justify-center"
            >
              <v-spacer class="mr-10"></v-spacer>
              {{ determineTicketType(current_ticket.type_ticket) }}
              <v-spacer></v-spacer>

              <v-btn class="ml-n2" icon dark @click="dialogFlag = false">
            ...
            <v-container class="mt-n10">
              ...
              <v-card-text>
                ...
                >
                  <v-col
                    cols="8"
                    style="font-size: 1.2rem; font-weight: bold; color: #3f505e"
                  >
                      ...
                      <v-stepper-header>
                        <template v-for="(n, index) in steps">
                          <v-stepper-step
                            ...
                            :step="`${index}`"
                            :complete="
                              current_step > index || ticket_status_number === 7
                            "
                            color="#366991"
                            style="font-size: 1em"
                          >
              ...
              <v-row
                v-if="
                  role !== 'student' &&
                  ticket_status_number !== 4 &&
                  ticket_status_number !== 6 &&
                  ticket_status_number !== 7
                "
...
export default {
  ...
  methods: {
    viewTicket(ticket) {
      ...
      // comments
      const commentsLength = Object.keys(
        this.current_ticket.descriptions
      ).length
      this.comments = []
      this.comments[0] = 'دانشجو : ' + ticket.ticketObject.message
      for (let i = 0; i < commentsLength; i++) {
        if (
          Object.values(this.current_ticket.descriptions)[i] !== null &&
          Object.values(this.current_ticket.descriptions)[i] !== ''
        ) {
          this.comments.push(
            Object.values(this.steps)[i + 1] +
              ' : ' +
              Object.values(this.current_ticket.descriptions)[i]
          )
        }
      ...
      this.ticket_status_text = this.checkStatus(
        this.current_ticket.status_step
      )
    ...
    // filterTicketType(value, search, item){
    //   console.log(value);
